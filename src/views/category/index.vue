  ...
  <div class="app-container">
      ...
      <el-col :span="12">
        <el-form :inline="true" :model="search" class="form-inline" size="small">
          <el-form-item label="关键字">
            <el-input v-model="search.kw" placeholder="输入分类" />
          </el-form-item>
          <el-form-item>
    ...
    >
      ...
      <el-table-column prop="catename" label="分类">
        <template slot-scope="scope">
          <el-input v-model="scope.row.catename" placeholder="输入分类" @change="handleChange(scope.row)" />
        </template>
      </el-table-column>
    ...
    <CategoryForm :dialog-info="dialogInfo" @refresh="fetchListData()" />
  </div>
</template>

<script>
import settings from '@/settings'
import { getCategoryList, updateCategory, destroyCategory } from '@/api/category'
...
export default {
  ...
  methods: {
    fetchListData() {
      getCategoryList(this.search).then(response => {
    ...
    handleDestroy(row) {
      this.$confirm('确定要删除吗?', '提示', {
        ...
        destroyCategory(row.id).then(response => {
          this.fetchListData()
        })
      })
    },
