<template>
  <div class="app-container">
      ...
      <el-col :span="12">
        <el-form :inline="true" :model="search" class="form-inline" size="small">
          <el-form-item label="关键字">
            <el-input v-model="search.kw" placeholder="输入角色名" />
          </el-form-item>
          <el-form-item>
    ...
    <RoleForm :dialog-info="dialogInfo" @refresh="fetchListData()" />
  </div>
</template>

<script>
import settings from '@/settings'
import { getRoleList, updateRole, destroyRole } from '@/api/role'
  ...
  methods: {
    ...
    handleDestroy(row) {
      this.$confirm('确定要删除吗?', '提示', {
        ...
        destroyRole(row.id).then(response => {
          this.fetchListData()
        })
      })
    },
